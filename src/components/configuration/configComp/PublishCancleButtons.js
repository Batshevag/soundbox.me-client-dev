...
function CollapseConfigurator(props) {
  ...
  const handlePublish = () => {
    if (window.location.pathname.includes("AddRecord")) {
      ...
      publishChannel(userName, channelName, channel);
    }
  };
    ...
    <div className="justify-content-center innerDivBottomConfiguratorFix">
      ...
      <div className="d-flex justify-content-center align-items-center ">
        <button
          ...
          className="innerDivBottomCancel iconBottom"
        >
          {window.location.pathname.includes("audio=") ? (<>Delete</>) : (<> Cancel</>)}
        </button>{" "}
      </div>
    </div>
  );
...
const mapDispatchToProps = (dispatch) => ({
  ...
  dispatchDeleteRecord: (audio) => dispatch(actions.deleteAudio(audio))
});
