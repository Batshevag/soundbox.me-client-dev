...
function PlayerProgress(props) {
    ...
    useEffect(() => {
        if (audioFromState.playNowAudioId!= undefined)
            setTitle(audioFromState.audioList.filter((a) => a._id === audioFromState.playNowAudioId)[0].title)
    })
    ...
    return (
        ...
        <div className="row">
            {!props.autoPlayPlayer ? <div className="col-2" style={{ paddingLeft: '10%', marginLeft: '1px' }}>
                ...
                <PodcastSharingOption url={URL} id={props.id} urlAudio={audioUrl} titleAudio={title} />
                </div>
