...
function PlayerButtons(props) {
  ...
  let audioUrlTemp = audioFromState.playNowAudioId
    ? audioFromState.audioList.filter(
        (a) => a._id === audioFromState.playNowAudioId
      )[0].audioUrl
    : "";
  const [audio, setAudio] = useState(
    new Audio(
      audioUrl && !props.autoPlayPlayer
        ? audioUrl
  ...
  useEffect(() => {
    audio.addEventListener("ended", () => {
      if (props.autoPlayPlayer) props.endAudioFunc(true);
  ...
  useEffect(() => {
    if (!props.autoPlayPlayer) setAudio(new Audio(audioUrl));
  }, [audioUrl]);

  // , 'audio/mp3'
  useEffect(() => {
    if (autoPlayPlayer) handlePause();
  }, [props.pause]);
    ...
    <>
      <div className="wrap_player" style={{ padding: "17px" }}>
        <div className="d-flex justify-content-center align-item-center">
          ...
          <Button
            onClick={handleStart}
            className="playButtons d-flex justify-content-center align-items-center"
          >
            <SkipPreviousRounded style={{ color: "#4B0083" }} />
          </Button>
          <Button
            onClick={() => {
              handlePause();
            ...
            className="playButtons pauseButton  d-flex justify-content-center align-items-center"
          >
          ...
          <Button
            ...
            className="playButtons d-flex justify-content-center align-items-center"
          >
            <SkipNextRounded style={{ color: " #4B0083" }} />
          </Button>
        </div>
        <div></div>
      </div>
